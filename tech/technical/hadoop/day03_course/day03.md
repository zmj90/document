...
8. **练习-创建外部表分区表**
   ...
   【1】创建数据存放目录
   	/weblog
   	/weblog/reporttime=2000-01-01 # 存放data1.txt数据
   	/weblog/reporttime=2000-01-02 # 存放data2.txt数据
   	hadoop fs -mkdir -p /weblog/reporttime=2000-01-01
   	hadoop fs -mkdir -p /weblog/reporttime=2000-01-02
   	hadoop fs -put data1.txt /weblog/reporttime=2000-01-01
   	hadoop fs -put data2.txt /weblog/reporttime=2000-01-02
   【2】准备两个文件
   	# data1.txt
   	1 rose 200
   	2 tom 100
   	...
   	5 nono 100
   	6 doudou 200
   【3】外部表 weblog_tab # 注意:1、location '/weblog' 2、添加分区字段
   	create external table weblog_tab(
   	id int,
   	name string,
   	score int
   	...
   	location '/weblog';
   【4】修复分区,并查询确认
   	msck repair table weblog_tab;
   【5】查询确认
   	select * from weblog_tab;
   ```

#### 6.3.5 分桶表

1. 分桶是相对分区进行更细粒度的划分。分桶将整个数据内容按照某列属性值的hash值进行区分，按照取模结果对数据分桶。如取模结果相同的数据记录存放到一个文件

2. 桶表也是一种用于优化查询而设计的表类型。创建桶表时，指定桶的个数、分桶的依据字段，hive就可以自动将数据分桶存储。查询时只需要遍历一个桶里的数据，或者遍历部分桶，这样就提高了查询效率

3. 关键字: clustered by(分桶字段) into 4 buckets
...
2. 创建t_user表并导入数据 - a
   ...
   load data local inpath '/home/tarena/data/users.txt' into table t_user;
   ```
...
3. 创建t_movie表并导入数据 - b
   ...
   load data local inpath '/home/tarena/data/movies.txt' into table t_movie;
   ```
...
4. 创建t_rating表并导入数据 - c
   ...
   load data local inpath '/home/tarena/data/ratings.txt' into table t_rating;
   ```
