...
![shujukuxitong](mysql-基础.assets/shujukuxitong.png)
...
![databases](mysql-基础.assets/databases.jpg)
...
![mysql](mysql-基础.assets/mysql.jpg)
...
![kujiegou](mysql-基础.assets/kujiegou.png)
...
![biaojiegou](mysql-基础.assets/biaojiegou.png)
...
![shuzi](mysql-基础.assets/shuzi.png)
...
![zifuchuan](mysql-基础.assets/zifuchuan.png)
...
![suanshu](mysql-基础.assets/suanshu.png)
...
[外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传(img-84nXCAWE-1610634103126)(img/比较.png)]

![bijiao](mysql-基础.assets/bijiao.png)
...
![luoji](mysql-基础.assets/luoji.png)
...
![yunsuanfu](mysql-基础.assets/yunsuanfu.png)
...
![shijian](mysql-基础.assets/shijian.png)
...
st_id int unique,
foreign key(st_id) references student(id) # 约束
on delete cascade 
on update cascade
);
 ```
...
![inner](mysql-基础.assets/inner.PNG)
...
![right](mysql-基础.assets/right.PNG)
...
[外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传(img-ulwANPsX-1610634103207)(img\QQ截图20200331224854.jpg)]
...
* **定义**： mysql数据库管理系统中用来处理表的处理器（操作数据库系统的核心代码）
...
4、已有表指定
   alter table 表名 engine=InnoDB; 也可以更改自增值
```
...
```mysql
1、支持行级锁,仅对指定的记录进行加锁，这样其它进程还是可以对同一个表中的其它记录进行操作。（不允许两个客户端操作同一行数据）
2、支持外键、事务、事务回滚
3、表字段和索引同存储在一个文件中
   1、表名.frm ：表结构
   2、表名.ibd : 表记录及索引文件
...
create user 'zmj'@'%' identified by 'zmj@2020';
3. 授权
   grant 权限列表 on 库.表 to "用户名"@"%" [identified by "密码"] with grant option;
grant select,insert,update,delete,create,drop,create user on *.* to 'zmj'@'%' with grant option;
   revoke insert,update,select on 库.表 from 'user'@'%';
revoke create,drop on *.* from 'zmj'@'%';
4. 刷新权限
   flush privileges;
5. 删除用户
   drop user "用户名"@"%"
...
[外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传(img-kWKdCyny-1610634103215)(img\QQ截图20200401233542.jpg)]

**权限列表**

```
all privileges 、select 、insert ，update，delete，alter,create,drop,create user等。
库.表 ： *.* 代表所有库的所有表
```
...
```python
# 查询数据
sql = "select name,age,sex,score from cls where score > 80;"
cur.execute(sql)
...
cur.fetchone() 获取查询结果集的第一条数据，查找到返回一个元组否则返回None """Fetch the next row"""
